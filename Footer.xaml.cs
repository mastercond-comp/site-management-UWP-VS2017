    ...
    public sealed partial class Footer : Page
    {
        public Footer()
        {
            ...
            private async void LoadFileTCatalog_Click(object sender, RoutedEventArgs e)
            {
                ...
                picker.FileTypeFilter.Add(".footer");
            ...
            private async void SaveFileKatalogTovarov_Click(object sender, RoutedEventArgs e)
            {
                ...
                savePicker4.FileTypeChoices.Add("Файл данных футера ", new List<string>() { ".footer" });
                ...
                if (Myfile4 != null)
                {
                    ...
                    if (status == Windows.Storage.Provider.FileUpdateStatus.Complete)
                    {
                        ...
                        this.StatusFile.Text = "Файл данных футера " + Myfile4.Name + " успешно сохранен.";
                        Progress.IsActive = false;

                    var messagedialog = new MessageDialog("Файл данных футера " + Myfile4.Name + " успешно сохранен.");
                    messagedialog.Commands.Add(new UICommand("Ok"));
                    await messagedialog.ShowAsync();
                }
                    else
                    {
                        this.StatusFile.Text = "Не удалось сохранить файл данных футера " + Myfile4.Name + ".";
                        Progress.IsActive = false;

                    var messagedialog = new MessageDialog("Не удалось сохранить файл данных футера " + Myfile4.Name + ".");
                    messagedialog.Commands.Add(new UICommand("Ok"));
                    await messagedialog.ShowAsync();
                }
                }
                ...
                {
                    this.StatusFile.Text = "Операция записи файла данных футера была прервана.";
                    Progress.IsActive = false;

                var messagedialog = new MessageDialog("Операция записи файла данных футера была прервана.");
                messagedialog.Commands.Add(new UICommand("Ok"));
                await messagedialog.ShowAsync();
            }
                Progress.IsActive = false;
            ...
            private async void SaveFileKatalogTovarovTPL_Click(object sender, RoutedEventArgs e)
            {
                ...
                savePicker4.SuggestedStartLocation = Windows.Storage.Pickers.PickerLocationId.DocumentsLibrary;
                // Поле выбора типа файла в диалоге
                savePicker4.FileTypeChoices.Add("TPL файл футера ", new List<string>() { ".tpl" });
                // Default file name if the user does not type one in or select a file to replace
                savePicker4.SuggestedFileName = "footer";
                //
                Windows.Storage.StorageFile Myfile4 = await savePicker4.PickSaveFileAsync();
                if (Myfile4 != null)
                {
                    ...
                    await FileIO.WriteTextAsync(Myfile4, "<!-- ======================================ФУТЕР======================================================== -->\r\n");
                    await FileIO.AppendTextAsync(Myfile4, "<footer style=\""+TextBoxSTYLEFooter.Text+"\">\r\n");
                    ...
                    await FileIO.AppendTextAsync(Myfile4, "</footer>\r\n");
                    await FileIO.AppendTextAsync(Myfile4, "<!-- ======================================КОНЕЦ ФУТЕРА================================================== -->\r\n");
                ...
                Windows.Storage.Provider.FileUpdateStatus status =
                    ...
                    if (status == Windows.Storage.Provider.FileUpdateStatus.Complete)
                    {
                        this.StatusFile.Text = "TPL файл футера " + Myfile4.Name + " успешно экспортирован.";
                    ...
                    var messagedialog = new MessageDialog("TPL-файл футера " + Myfile4.Name + " успешно экспортирован.");
                    messagedialog.Commands.Add(new UICommand("Ok"));
                    await messagedialog.ShowAsync();
                    ...
                    {
                        this.StatusFile.Text = "Не удалось сохранить TPL-файл футера " + Myfile4.Name + ".";
                        Progress.IsActive = false;

                    var messagedialog = new MessageDialog("Не удалось сохранить TPL-файл футера " + Myfile4.Name + ".");
                    messagedialog.Commands.Add(new UICommand("Ok"));
                    await messagedialog.ShowAsync();
                }
                }
                ...
                {
                    this.StatusFile.Text = "Операция записи TPL-файла футера была прервана.";
                    Progress.IsActive = false;

                var messagedialog = new MessageDialog("Операция записи TPL-файла футера была прервана.");
                messagedialog.Commands.Add(new UICommand("Ok"));
                await messagedialog.ShowAsync();
            }
                Progress.IsActive = false;
            ...
            await FileIO.WriteTextAsync(UPLOADFile, "<!-- ======================================ФУТЕР======================================================== -->\r\n");
            await FileIO.AppendTextAsync(UPLOADFile, "<footer style=\"" + TextBoxSTYLEFooter.Text + "\">\r\n");
            ...
            await FileIO.AppendTextAsync(UPLOADFile, "</footer>\r\n");
            await FileIO.AppendTextAsync(UPLOADFile, "<!-- ======================================КОНЕЦ ФУТЕРА================================================== -->\r\n");
