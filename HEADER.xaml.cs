...
namespace Управление.сайтом
{
    ...
    public sealed partial class HEADER : Page
    {
        public HEADER()
        {
            ...
            TextBoxCODEFooter.Text = "<!DOCTYPE html><html lang=\"ru\"><head><meta charset=\"utf-8\"><meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><meta http-equiv=\"Cache-Control\" content=\"no-cache\"><!-- The above 3 meta tags *must* come first in the head; any other head content must come *after* these tags --><link rel=\"shortcut icon\" href=\"/favicon.png\"><!-- Bootstrap core CSS --><link href=\"/css/bootstrap.min.css\" rel=\"stylesheet\"><!-- IE10 viewport hack for Surface/desktop Windows 8 bug --><link href=\"/css/ie10-viewport-bug-workaround.css\" rel=\"stylesheet\"><script src=\"/assets/js/ie-emulation-modes-warning.js\"></script><!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries --><!--[if lt IE 9]><script src=\"https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js\"></script><script src=\"https://oss.maxcdn.com/respond/1.4.2/respond.min.js\"></script><![endif]--><!-- Custom styles for this template здесь --><link href=\"/css/header.css\" rel=\"stylesheet\"><link href=\"/css/carousel.css\" rel=\"stylesheet\">";
        }
        ...
        private async void LoadFileTCatalog_Click(object sender, RoutedEventArgs e)
        {
            ...
            picker.FileTypeFilter.Add(".header");
        ...
        private async void SaveFileKatalogTovarov_Click(object sender, RoutedEventArgs e)
        {
            ...
            savePicker4.FileTypeChoices.Add("Файл данных HEADER ", new List<string>() { ".header" });
            ...
            if (Myfile4 != null)
            {
                ...
                if (status == Windows.Storage.Provider.FileUpdateStatus.Complete)
                {
                    ...
                    this.StatusFile.Text = "Файл данных HEADER " + Myfile4.Name + " успешно сохранен.";
                    Progress.IsActive = false;

                    var messagedialog = new MessageDialog("Файл данных HEADER " + Myfile4.Name + " успешно сохранен.");
                    messagedialog.Commands.Add(new UICommand("Ok"));
                    await messagedialog.ShowAsync();
                }
                else
                {
                    this.StatusFile.Text = "Не удалось сохранить файл данных HEADER " + Myfile4.Name + ".";
                    Progress.IsActive = false;

                    var messagedialog = new MessageDialog("Не удалось сохранить файл данных HEADER " + Myfile4.Name + ".");
                    messagedialog.Commands.Add(new UICommand("Ok"));
                    await messagedialog.ShowAsync();
                }
            }
            ...
            {
                this.StatusFile.Text = "Операция записи файла данных HEADER была прервана.";
                Progress.IsActive = false;

                var messagedialog = new MessageDialog("Операция записи файла данных HEADER была прервана.");
                messagedialog.Commands.Add(new UICommand("Ok"));
                await messagedialog.ShowAsync();
            }
            Progress.IsActive = false;
        ...
        private async void SaveFileKatalogTovarovTPL_Click(object sender, RoutedEventArgs e)
        {
            ...
            savePicker4.SuggestedStartLocation = Windows.Storage.Pickers.PickerLocationId.DocumentsLibrary;
            // Поле выбора типа файла в диалоге
            savePicker4.FileTypeChoices.Add("TPL файл модуля HEADER ", new List<string>() { ".tpl" });
            // Default file name if the user does not type one in or select a file to replace
            savePicker4.SuggestedFileName = "header";
            //
            Windows.Storage.StorageFile Myfile4 = await savePicker4.PickSaveFileAsync();
            if (Myfile4 != null)
            {
                ...
                if (status == Windows.Storage.Provider.FileUpdateStatus.Complete)
                {
                    this.StatusFile.Text = "TPL файл модуля HEADER " + Myfile4.Name + " успешно экспортирован.";
                    ...
                    var messagedialog = new MessageDialog("TPL файл модуля HEADER " + Myfile4.Name + " успешно экспортирован.");
                    messagedialog.Commands.Add(new UICommand("Ok"));
                    await messagedialog.ShowAsync();
                ...
                {
                    this.StatusFile.Text = "Не удалось сохранить TPL-файл модуля HEADER " + Myfile4.Name + ".";
                    Progress.IsActive = false;

                    var messagedialog = new MessageDialog("Не удалось сохранить TPL-файл модуля HEADER " + Myfile4.Name + ".");
                    messagedialog.Commands.Add(new UICommand("Ok"));
                    await messagedialog.ShowAsync();
                }
            }
            ...
            {
                this.StatusFile.Text = "Операция записи TPL-файла модуля HEADER была прервана.";
                Progress.IsActive = false;

                var messagedialog = new MessageDialog("Операция записи TPL-файла модуля HEADER была прервана.");
                messagedialog.Commands.Add(new UICommand("Ok"));
                await messagedialog.ShowAsync();
            }
            Progress.IsActive = false;
        ...
        private async void AppBarButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            savePicker4.SuggestedStartLocation = Windows.Storage.Pickers.PickerLocationId.DocumentsLibrary;
            // Поле выбора типа файла в диалоге
            savePicker4.FileTypeChoices.Add("Файл данных модуля HEADER ", new List<string>() { ".header" });
            // Default file name if the user does not type one in or select a file to replace
            savePicker4.SuggestedFileName = "header";
            //
            Windows.Storage.StorageFile Myfile4 = await savePicker4.PickSaveFileAsync();
